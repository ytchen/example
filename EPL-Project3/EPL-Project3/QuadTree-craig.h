...
public:
  ...
  List<Obj> nearby(const Point& center, double radius) const; 
                                // return a List of Objs that are within 
...
template <class Obj> 
class TreeNode {
  typedef pair<bool, Procedure0> Result;
...
public:
  ...
  /*
   * return the list of objects (not including one at 'center') that
   * are inside this region, and also not more than 'dist' units
   * away from 'center'
   */
  void find_nearby(List<Obj>& list, const Point& center, double dist) const {
    if (is_empty()) return;
   ...
   *   'center.position()'
   */
  pair<bool,Obj> closest(const Point& center, double& dist) const {
    /* three cases, 0 objects, 1 object or more than one object
       are in this region */
    if (! intersects(center, dist)) 
      return pair<bool,Obj>(false, Obj());
...
Obj QuadTree<Obj>::closest(const Point& pos) const {
  double dist = HUGE;
  pair<bool,Obj> tmp = root->closest(pos, dist);
  assert(tmp.first);
  return tmp.second;
}

template <class Obj>
List<Obj> QuadTree<Obj>::nearby(const Point& pos, double dist) const {
  List<Obj> result;
...
void QuadTree<Obj>::update_position(const Point& pos_old, 
  ...
  pair<TreeNode<Obj>*, TreeNode<Obj>*> res = root->find_leaf(pos_old);
