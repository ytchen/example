...
bool YiTing::huntPrey(const ObjList& prey)
{
	...
	for(ObjList::const_iterator thePrey = prey.begin(); thePrey != prey.end(); ++thePrey)
	{
		if ((thePrey->their_speed < 0.001 && thePrey->species == fav_food) ||
			(checkEnemy(thePrey->their_speed) && canEat(*thePrey)))
		{
			...
			{
				curSpeed = 2 + myTailSpeed;
				set_speed(2 + myTailSpeed);
			}
			myDelete(huntEvent);
