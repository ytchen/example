...
#endif
Initializer<MChenCipher> __MChenCipher_initializer;

string MChenCipher::species_name(void) const
{
	if(bAttack)
		return "Algae";
	else
...
string MChenCipher::player_name(void) const
{
	return "MChenCipher";
}

Action MChenCipher::encounter(const ObjInfo& info)
{

	if(HitObj==info){
		countADhunt++;
		//_DPRINTF(("[MChenCipher] Event ad %d  success %d",countAD,countADhunt));
	}
	...
	if (info.species == player_name()) {  //myslef   
		...
		hunt_event = new Event(10.1, make_procedure(*this, &MChenCipher::hunt));
		return LIFEFORM_IGNORE;
	}
	//	else if(info.species=="Algae" && my_verify(decripted(myspeed))){  //myslef   
	//		set_course(info.bearing + M_PI);
	//		hunt_event = new Event(10.2, make_procedure(*this, &MChenCipher::hunt));
	//		return LIFEFORM_IGNORE;
	//	}
	else {
		myDelete( hunt_event);
		if(encounter_strategy==BIG_GUY_WINS){
			if(health()>info.health){
				hunt_event_encounter = new Event(0.4, make_procedure(*this, &MChenCipher::HuntEncounter));
			}
			else{
				...
				hunt_event = new Event(8, make_procedure(*this, &MChenCipher::hunt));
			...
			if(health()<info.health){
				...
				if(health()*0.5<info.health){
					//spawn();
					MChenCipher* child = new MChenCipher;
					reproduce(child);
					hunt_event_encounter = new Event(0.4, make_procedure(*this, &MChenCipher::HuntEncounter));
				}
				else{
			...
			hunt_event_encounter = new Event(0.4, make_procedure(*this, &MChenCipher::HuntEncounter));  //pursuit if enemy survied 
		}
		else if(encounter_strategy==SLOWER_GUY_WINS){
			myspeed=0.0;
			set_speed(-20.0);

			hunt_event_encounter = new Event(min_delta_time, make_procedure(*this, &MChenCipher::HuntEncounter)); //pursuit if enemy survied 
		}
		else {
			//if(health()>info.health){
			//		hunt_event = new Event(0.4, make_procedure(*this, &MChenCipher::hunt));
			//}
			///else

			hunt_event = new Event(10.0, make_procedure(*this, &MChenCipher::hunt));
...
void MChenCipher::initialize(void)
{
	LifeForm::add_creator(MChenCipher::create, "MChenCipher");
}
...
*/
MChenCipher::MChenCipher()
{
	hunt_event = Nil<Event>();
	hunt_event_encounter = Nil<Event>();
	(void) new Event(0.0, make_procedure(*this, &MChenCipher::live));
	Tsplit = 1 ;
	countADhunt=0;
	HitObj =ObjInfo();
	countAD=0;
...
Color MChenCipher::my_color(void) const
{
	return RED;
}  

LifeForm* MChenCipher::create(void)
{
	MChenCipher* res = Nil<MChenCipher>();
	res = new MChenCipher;
	res->display();
	return res;
}
...
void MChenCipher::live(void)
{
	...
	set_speed(myspeed);
	hunt_event = new Event(1.0, make_procedure(*this, &MChenCipher::hunt));
}
void MChenCipher::HuntEncounter(void)
{
	countAD++;
	...
	hunt_event = new Event(1.5, make_procedure(*this, &MChenCipher::hunt));



}
void MChenCipher::hunt(void)
{
	...
	if(!bAttack){
		...
		//	myspeed=encripted(myspeed);
		double dt=best_d/myspeed;
		dt=9>dt?9:dt;
		hunt_event = new Event(dt, make_procedure(*this, &MChenCipher::hunt));
				...
				if (!my_verify(decripted((*i).their_speed) )&& (*i).species!=player_name() ) { 
					...
					if(encounter_strategy==BIG_GUY_WINS){
						if(health()>(*i).health/*&& (*i).health>0.4*/){
							ds=(*i).distance;
							//_DPRINTF(("[MChenCipher] attacker enemy en %f  speed %f  course %f  bear%f ",(*i).health,(*i).their_speed,(*i).their_course,(*i).bearing));
							course_changed = 0 ;
							if (best_d > ds) {
								set_course((*i).bearing);
								mycourse=(*i).bearing;
...
void MChenCipher::ST(bool bspe){
	double ds(0.0);
	const string fav_food = "Algae";

	//_DPRINTF(("[MChenCipher]Large search"));
	hunt_event = Nil<Event>();
	...
	for (ObjList::iterator i = prey.begin(); i != prey.end();  ++i) {
		...
		if (bspe && (*i).species == fav_food && (*i).their_speed==0 ) { // True Algae
			countSpe++;
	...
	if(bspe&&speMax>0){
		...
		if(dt>dTimeFre)
			dt=dTimeFre;
		//_DPRINTF(("[MChenCipher]Large search max %d index:%d crouse %f",speMax,speIndex,speCR[speIndex]));
		hunt_event = new Event(10, make_procedure(*this, &MChenCipher::hunt));
	}
		...
		if(dt>dTimeFre)
			dt=dTimeFre;
		//_DPRINTF(("[MChenCipher]Large search Max%d index:%d dist %f",genMax,Index,CR[Index]));
		hunt_event = new Event((10+5*attackercount), make_procedure(*this, &MChenCipher::hunt));
	}
	else{
		attackercount++;
		evalution*=0.6;
		...
		myspeed=encripted(myspeed);
		set_speed(myspeed);
		hunt_event = new Event((10+10*attackercount), make_procedure(*this, &MChenCipher::hunt));
	}

}
void MChenCipher::SetSpeed(){
